...
class Longformer(RobertaModel):
    def __init__(self,config):
        ...
        if config.attention_mode=='n2':
            pass #Use RoBerta Self Attention
        else:
            for i, layer in enumerate(self.encoder.layer):
                layer.attention.self = LongformerSelfAttention(config,layers_id=i)

class LongformerConfig(RobertaConfig):
    def __init__(self,attention_window:List[int]=None,attention_dilation:List[int]=None,
                 autoregressive:bool=False,attention_model:str='sliding_chunks',**kwargs):
        super().__init__(**kwargs)
        self.attention_window=attention_window
        self.attention_model = attention_model
        self.attention_dilation=attention_dilation
        self.autoregressive=autoregressive
        assert self.attention_model in ['sliding_chunks','n2','sliding_chunks_no_overlap']

class LongformerSelfAttention(nn.Module):
    def __init__(self,config,layers_id):
        super().__init__()
        torch.manual_seed(1)
        ...
        self.attention_window = config.attention_window[self.layers_id]
        self.attention_dilation = config.attention_dilation[self.layers_id]
